...
Local-only builds do not surface provider usage/quota; `/status` shows local tokens only.

Surfaces:

- `/status` (shows local token counts)
- `openclaw status --usage` (prints local usage totals)
- macOS menu bar (Usage section under Context)

Notes:

- Data comes from local session logs (no provider usage endpoints).
...
Note: model scanning is disabled in the local-only build.
