...
- **Model choice matters:** older/legacy models can be less robust against prompt injection and tool misuse. Prefer modern, instruction-hardened models for any bot with tools. For local-only builds, start with `ollama/gpt-oss-120b` and evaluate other local models before exposing tools broadly.
