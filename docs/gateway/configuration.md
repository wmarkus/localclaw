...
```json5
{
  models: {
    providers: {
      ollama: {
        baseUrl: "http://127.0.0.1:${OLLAMA_PORT}/v1",
      },
    },
  },
}
...
Local-only builds do not require model auth profiles. You can omit this section
unless you are maintaining legacy metadata for older setups.
...
| ----------------- | ---------------------- | ----------------------- |
| `{model}`         | Short model name       | `gpt-oss-120b`          |
| `{modelFull}`     | Full model identifier  | `ollama/gpt-oss-120b`   |
...
Example output: `[gpt-oss-120b | think:high] Here's my response...`
...
Text-to-speech is disabled in the local-only build. `messages.tts` settings are
accepted for compatibility but ignored, and `/tts` commands report TTS as unavailable.

### `talk`

Defaults for Talk mode (macOS/iOS/Android). Talk is driven by connected mobile nodes.
If your Talk node requires a voice provider API key, set `talk.apiKey` accordingly.
...
OpenClaw ships a few built-in alias shorthands. Defaults only apply when the model
...
`agents.defaults.model.primary` should be set as `provider/model` (e.g. `ollama/gpt-oss-120b`).
Aliases come from `agents.defaults.models.*.alias` (e.g. `oss`).
If you omit the provider, OpenClaw will warn and assume `ollama/<model>` for now.
...
- `applyPatch.enabled`: enable experimental `apply_patch` (models that support apply_patch; default false)
- `applyPatch.allowModels`: optional allowlist of model ids (e.g. `gpt-oss-120b` or `ollama/gpt-oss-120b`)
...
- Each `models[]` entry:
  - CLI entry (`type: "cli"` or when `command` is set):
    - `command`: executable to run.
    - `args`: templated args (supports `{{MediaPath}}`, `{{Prompt}}`, `{{MaxChars}}`, etc).
  - Provider entry (`type: "provider"` or omitted) is **not supported** in the local-only build and
    is ignored unless you supply a custom local provider implementation.
...
Provider keys accept either `provider` (e.g. `ollama`) or `provider/model`
(e.g. `ollama/gpt-oss-120b`).

Example (keep global coding profile, but minimal tools for Ollama):
...
```json5
{
  tools: {
    ...
    byProvider: {
      "ollama/gpt-oss-120b": { allow: ["group:fs", "sessions_list"] },
    },
  },
}
```
...
### `models` (local-only catalog)

OpenClaw uses a local model catalog stored at
`~/.openclaw/agents/<agentId>/agent/models.json`. On startup, the gateway writes or
merges this file using local Ollama discovery plus any explicit `models.providers.ollama`
...
Only the `ollama` provider is supported in the local-only build. Any other provider id
will fail at runtime.

Select the model via `agents.defaults.model.primary` (`ollama/<model>`).
...
- `apiKey`: optional convenience for skills that declare a primary env var (e.g. a local API token).
...
```json5
{
  skills: {
    ...
    entries: {
      "local-helper": {
        env: {
          LOCAL_HELPER_BIN: "/usr/local/bin/local-helper",
        },
      },
...
```json5
{
  plugins: {
    ...
    load: {
      paths: ["~/Projects/oss/example-plugin"],
    },
    entries: {
      "example-plugin": {
        enabled: true,
