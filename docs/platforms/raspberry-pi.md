...
Local-only builds require a **local** model server (Ollama). A Raspberry Pi is
generally **not** suitable for large models like `gpt-oss-120b`.

If you must run on a Pi, use a much smaller model and expect slow performance.
For the best experience, run OpenClaw + Ollama on a more capable host.
