...
- Model runtime: local Ollama (loopback-only)
...
- **Prefer env vars over config file** for tokens. This keeps secrets out of `openclaw.json` where they could be accidentally exposed or logged.
- Local-only builds require Ollama on the same host (loopback only). If you deploy to Fly, run Ollama inside the VM and keep it bound to `127.0.0.1`.
...
If a plugin or integration requires inbound webhooks, run a tunnel inside the container (ngrok) or expose a path via Tailscale Funnel. Restrict allowed hosts to the public tunnel hostname.
