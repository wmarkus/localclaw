...
---
name: local-helper
description: Use a local CLI helper for custom tasks.
---
```
    ...
    "openclaw":
      {
        "requires":
          {
            "bins": ["local-helper"],
            "env": ["LOCAL_HELPER_TOKEN"],
            "config": ["browser.enabled"],
          },
        "primaryEnv": "LOCAL_HELPER_TOKEN",
      },
  }
---
```
...
- If an agent is sandboxed, the binary must also exist **inside the container**.
  ...
  Example: a skill that depends on `local-helper` needs the `local-helper` CLI
...
metadata:
  {
    "openclaw":
      {
        "emoji": "♊️",
        "requires": { "bins": ["local-helper"] },
        "install":
          [
            {
              "id": "brew",
              "kind": "brew",
              "formula": "local-helper",
              "bins": ["local-helper"],
              "label": "Install local helper (brew)",
            },
          ],
      },
  }
...
```json5
{
  skills: {
    entries: {
      "local-helper": {
        enabled: true,
        apiKey: "LOCAL_HELPER_TOKEN_HERE",
        env: {
          LOCAL_HELPER_TOKEN: "LOCAL_HELPER_TOKEN_HERE",
        },
        config: {
...
- Token counts vary by model tokenizer. A rough estimate is ~4 chars/token, so **97 chars ≈ 24 tokens** per skill plus your actual field lengths.
