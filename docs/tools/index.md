...
`provider/model` (e.g. `ollama/gpt-oss-120b`).
...
tool usage guidance is injected into prompts.
...
Experimental: enable via `tools.exec.applyPatch.enabled` (models that support apply_patch).
