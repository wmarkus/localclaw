...
- Model profile selection with safe defaults
...
│   ├── extra-params.ts            # Model-specific stream params
...
6. **Schema Normalization**: Schemas cleaned for strict tool parsers
...
This ensures OpenClaw's policy filtering, sandbox integration, and extended toolset remain consistent across models.
...
- Refusal string scrubbing
- Turn ordering checks for strict parsers
- Tool schema sanitization
- Session history sanitization
- `apply_patch` tool support (when enabled)
...
5. **Model quirks**: Model-specific codepaths that pi could potentially handle

## Tests

Tests that cover the pi integration and its extensions live under:
