...
export function createLlmTaskTool(api: OpenClawPluginApi) {
  return {
    ...
    parameters: Type.Object({
      ...
      provider: Type.Optional(Type.String({ description: "Provider override (ollama only)." })),
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      if (provider !== "ollama") {
        throw new Error(`llm-task only supports the local ollama provider (got ${provider}).`);
      }
