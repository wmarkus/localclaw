...
function fakeApi(overrides: any = {}) {
  return {
    ...
    source: "test",
    config: {
      agents: { defaults: { workspace: "/tmp", model: { primary: "ollama/gpt-oss-120b" } } },
    },
    pluginConfig: {},
...
describe("llm-task tool (json-only)", () => {
  ...
  it("passes provider/model overrides to embedded runner", async () => {
    ...
    const tool = createLlmTaskTool(fakeApi());
    await tool.execute("id", { prompt: "x", provider: "ollama", model: "gpt-oss-120b" });
    // oxlint-disable-next-line typescript/no-explicit-any
  ...
  it("enforces allowedModels", async () => {
    ...
    const tool = createLlmTaskTool(
      fakeApi({ pluginConfig: { allowedModels: ["ollama/gpt-oss-120b"] } }),
    );
    await expect(
      tool.execute("id", { prompt: "x", provider: "ollama", model: "gpt-oss-7b" }),
