...
function resolveThinkLevelOptions(): readonly string[] {
  return THINK_LEVELS;
}

function resolveThinkLevelDisplay(value: string): string {
  return value;
}
...
function renderRow(
  ...
  const thinking = resolveThinkLevelDisplay(rawThinking);
  const thinkLevels = resolveThinkLevelOptions();
