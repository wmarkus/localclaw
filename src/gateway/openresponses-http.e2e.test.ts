...
describe("OpenResponses HTTP API (e2e)", () => {
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      mockAgentOnce([{ text: "I am a local model" }]);
      const resHistory = await postResponses(port, {
        model: "openclaw",
        input: [
          { type: "message", role: "system", content: "You are a helpful assistant." },
          { type: "message", role: "user", content: "Hello, who are you?" },
          { type: "message", role: "assistant", content: "I am a local model." },
      ...
      expect(historyMessage).toContain("Assistant: I am a local model.");
