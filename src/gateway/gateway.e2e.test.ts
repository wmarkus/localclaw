...
import { installOllamaCompletionsMock } from "./test-helpers.ollama-mock.js";
...
describe("gateway e2e", () => {
  it(
    "runs a mock local tool call end-to-end via gateway agent loop",
    { timeout: 90_000 },
    async () => {
      const prev = {
      ...
      const { baseUrl: ollamaBaseUrl, restore } = installOllamaCompletionsMock();
      ...
      const client = await connectGatewayClient({
        ...
        token,
        clientDisplayName: "vitest-mock-ollama",
      });

      try {
        const sessionKey = "agent:dev:mock-ollama";
        ...
        client.stop();
        await server.close({ reason: "mock ollama test complete" });
