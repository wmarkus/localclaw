...
describe("gateway server sessions", () => {
  ...
  test("lists and patches session store via sessions.* RPC", async () => {
    ...
    piSdkMock.models = [{ id: "gpt-oss-120b", name: "GPT OSS 120B", provider: "ollama" }];
    ...
    expect(modelPatched.payload?.entry.modelOverride).toBe("gpt-oss-120b");
    expect(modelPatched.payload?.entry.providerOverride).toBe("ollama");
