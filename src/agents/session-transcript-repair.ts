...
export function repairToolUseResultPairing(messages: AgentMessage[]): ToolUseRepairReport {
  // Strict tool parsers reject transcripts where assistant tool calls are not
  ...
  for (let i = 0; i < messages.length; i += 1) {
    ...
    if (role !== "assistant") {
      ...
      // Any "free-floating" toolResult entries in session history can make strict
      // tool parsers reject the entire request.
      if (role !== "toolResult") {
        out.push(msg);
      } else {
