...
export function resolveConfiguredModelRef(params: {
  ...
  if (rawModel) {
    ...
    if (!trimmed.includes("/")) {
      ...
      // Default to ollama if no provider is specified, but warn as this is deprecated.
      console.warn(
        `[openclaw] Model "${trimmed}" specified without provider. Falling back to "ollama/${trimmed}". Please use "ollama/${trimmed}" in your config.`,
      );
