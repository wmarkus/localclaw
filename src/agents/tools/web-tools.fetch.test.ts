...
describe("web_fetch extraction fallbacks", () => {
  ...
  it("enforces maxChars after wrapping", async () => {
    ...
    const tool = createWebFetchTool({
      config: {
        tools: {
          web: {
            fetch: { cacheTtlMinutes: 0, maxChars: 2000 },
          },
        },
      },
      sandboxed: false,
  ...
  it("honors maxChars even when wrapper overhead exceeds limit", async () => {
    ...
    const tool = createWebFetchTool({
      config: {
        tools: {
          web: {
            fetch: { cacheTtlMinutes: 0, maxChars: 100 },
          },
        },
      },
      sandboxed: false,
  ...
  it("throws when readability returns no content", async () => {
    const mockFetch = vi.fn((input: RequestInfo) =>
      Promise.resolve(
        htmlResponse("<!doctype html><html><head></head><body></body></html>", requestUrl(input)),
      ),
    );
    // @ts-expect-error mock fetch
    global.fetch = mockFetch;
  ...
  it("throws when readability is disabled", async () => {
    ...
    const tool = createWebFetchTool({
      config: {
        tools: {
          web: {
            fetch: { readability: false, cacheTtlMinutes: 0 },
          },
        },
      },
      sandboxed: false,
  ...
  it("wraps external content and clamps oversized maxChars", async () => {
    ...
    const tool = createWebFetchTool({
      config: {
        tools: {
          web: {
            fetch: { cacheTtlMinutes: 0, maxCharsCap: 10_000 },
          },
        },
      },
      sandboxed: false,
