...
import { cleanToolSchema, normalizeToolParameters } from "./pi-tools.schema.js";
import {
...
export function createOpenClawCodingTools(options?: {
  ...
  /**
   * Provider of the currently selected model (used for tool allowlist gating).
   * Example: "ollama".
   */
  modelProvider?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    ...
    if (tool.name === "write") {
      ...
      return [
        wrapToolParamNormalization(createWriteTool(workspaceRoot), REQUIRED_PARAM_GROUPS.write),
      ];
    }
    if (tool.name === "edit") {
      if (sandboxRoot) {
      ...
      return [
        wrapToolParamNormalization(createEditTool(workspaceRoot), REQUIRED_PARAM_GROUPS.edit),
      ];
    }
    return [tool];
  });
  ...
  // Without this, some tool parsers will reject root-level union schemas.
  ...
  // NOTE: Keep canonical (lowercase) tool names here to avoid transport-specific remaps.
  return withAbort;
}
