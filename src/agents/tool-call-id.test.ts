...
import { isValidToolCallId, sanitizeToolCallIds } from "./tool-call-id.js";
...
describe("sanitizeToolCallIds", () => {
  describe("strict mode (default)", () => {
    it("is a no-op for already-valid non-colliding IDs", () => {
      ...
      const out = sanitizeToolCallIds(input);
    ...
    it("strips non-alphanumeric characters from tool call IDs", () => {
      ...
      expect(isValidToolCallId(toolCall.id as string, "strict")).toBe(true);
    ...
    it("avoids collisions when sanitization would produce duplicate IDs", () => {
      ...
      expect(isValidToolCallId(a.id as string, "strict")).toBe(true);
      expect(isValidToolCallId(b.id as string, "strict")).toBe(true);
  ...
  describe("strict mode (alphanumeric only)", () => {
    it("strips underscores and hyphens from tool call IDs", () => {
      ...
      const out = sanitizeToolCallIds(input, "strict");
      expect(out).not.toBe(input);
  ...
  describe("strict9 mode (fixed-length tool call IDs)", () => {
    it("enforces alphanumeric IDs with length 9", () => {
      ...
      const out = sanitizeToolCallIds(input, "strict9");
      expect(out).not.toBe(input);
      ...
      expect(isValidToolCallId(a.id as string, "strict9")).toBe(true);
      expect(isValidToolCallId(b.id as string, "strict9")).toBe(true);
