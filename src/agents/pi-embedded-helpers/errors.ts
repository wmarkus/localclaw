...
const ERROR_PAYLOAD_PREFIX_RE = /^(?:error|api\s*error|apierror|gateway\s*error)[:\s-]+/i;
const FINAL_TAG_RE = /<\s*\/?\s*final\s*>/gi;
const ERROR_PREFIX_RE =
  /^(?:error|api\s*error|gateway\s*error|request failed|failed|exception)[:\s-]+/i;
...
export function isFormatError(raw: string): boolean {
