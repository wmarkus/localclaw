 ...
 * In some runtimes, tool calls without `thought_signature` are downgraded to
 * text blocks like `[Tool Call: name (ID: ...)]`. These should not be shown to
 * users.
 */
...
export function extractAssistantText(msg: AssistantMessage): string {
  ...
  const blocks = Array.isArray(msg.content)
    ? msg.content
        .filter(isTextBlock)
        .map((c) => stripThinkingTagsFromText(stripDowngradedToolCallText(c.text)).trim())
        .filter(Boolean)
    : [];
...
export function splitThinkingTaggedText(text: string): ThinkTaggedSplitBlock[] | null {
  ...
  // with a think tag (common for local runtimes that emulate reasoning blocks
  // via tags).
