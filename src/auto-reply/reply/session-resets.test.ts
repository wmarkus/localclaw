...
vi.mock("../../agents/model-catalog.js", () => ({
  loadModelCatalog: vi.fn(async () => [
    { provider: "ollama", id: "m2.1", name: "M2.1" },
    { provider: "ollama", id: "gpt-4o-mini", name: "GPT-4o mini" },
  ]),
}));
...
describe("applyResetModelOverride", () => {
  it("selects a model hint and strips it from the body", async () => {
    ...
    const aliasIndex = buildModelAliasIndex({ cfg, defaultProvider: "ollama" });
    const sessionEntry = {
      sessionId: "s1",
      updatedAt: Date.now(),
    };
    const sessionStore = { "agent:main:dm:1": sessionEntry };
    const sessionCtx = { BodyStripped: "ollama summarize" };
    const ctx = { ChatType: "direct" };

    await applyResetModelOverride({
      cfg,
      resetTriggered: true,
      bodyStripped: "ollama summarize",
      sessionCtx,
      ctx,
      sessionEntry,
      sessionStore,
  ...
  it("skips when resetTriggered is false", async () => {
    ...
    expect(sessionCtx.BodyStripped).toBe("ollama summarize");
  });
});
