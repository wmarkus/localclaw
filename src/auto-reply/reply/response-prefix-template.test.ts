...
describe("resolveResponsePrefixTemplate", () => {
  ...
  it("returns template as-is when no variables present", () => {
    expect(resolveResponsePrefixTemplate("[Local]", {})).toBe("[Local]");
  });
  ...
  it("resolves {model} variable", () => {
    ...
    expect(result).toBe("[gpt-oss-120b]");
  });

  it("resolves {modelFull} variable", () => {
    const result = resolveResponsePrefixTemplate("[{modelFull}]", {
      modelFull: "ollama/gpt-oss-120b",
    });
    expect(result).toBe("[ollama/gpt-oss-120b]");
  });
  ...
  it("resolves multiple variables", () => {
    ...
    expect(result).toBe("[gpt-oss-120b | think:high]");
  });
  ...
  it("handles case insensitivity", () => {
    ...
    expect(result).toBe("[gpt-oss-120b | low]");
  });
  ...
  it("handles mixed resolved and unresolved variables", () => {
    ...
    expect(result).toBe("[gpt-oss-120b | {provider}]");
  });
  ...
  it("handles complex template with all variables", () => {
    ...
    expect(result).toBe("[OpenClaw] ollama/gpt-oss-120b (think:high)");
  });
});

describe("extractShortModelName", () => {
  it("strips provider prefix", () => {
    expect(extractShortModelName("ollama/gpt-oss-120b")).toBe("gpt-oss-120b");
    expect(extractShortModelName("ollama/gpt-oss-120b-latest")).toBe("gpt-oss-120b");
  });

  it("strips date suffix", () => {
    expect(extractShortModelName("gpt-oss-120b-20251101")).toBe("gpt-oss-120b");
    expect(extractShortModelName("gpt-oss-120b-20250115")).toBe("gpt-oss-120b");
  });

  it("strips -latest suffix", () => {
    expect(extractShortModelName("gpt-oss-120b-latest")).toBe("gpt-oss-120b");
    expect(extractShortModelName("gpt-oss-120b-latest")).toBe("gpt-oss-120b");
  });

  it("handles model without provider", () => {
    expect(extractShortModelName("gpt-oss-120b")).toBe("gpt-oss-120b");
  });

  it("handles full path with provider and date suffix", () => {
    expect(extractShortModelName("ollama/gpt-oss-120b-20251101")).toBe("gpt-oss-120b");
  });
...
describe("hasTemplateVariables", () => {
  ...
  it("returns false for static prefix", () => {
    expect(hasTemplateVariables("[Local]")).toBe(false);
  });
