...
vi.mock("../../agents/model-catalog.js", () => ({
  loadModelCatalog: vi.fn(async () => [
    ...
    { provider: "ollama", id: "gpt-oss-7b", name: "GPT OSS 7B" },
  ]),
}));
...
describe("/models command", () => {
  ...
  it("lists provider models with pagination hints", async () => {
    ...
    const params = buildParams("/models ollama", cfg, { Surface: "discord" });
    const result = await handleCommands(params);
    expect(result.shouldContinue).toBe(false);
    expect(result.reply?.text).toContain("Models (ollama)");
    expect(result.reply?.text).toContain("page 1/");
    expect(result.reply?.text).toContain("ollama/gpt-oss-120b");
    expect(result.reply?.text).toContain("Switch: /model <provider/model>");
    expect(result.reply?.text).toContain("All: /models ollama all");
  });

  it("ignores page argument when all flag is present", async () => {
    // Use discord surface for text-based output tests
    const params = buildParams("/models ollama 3 all", cfg, { Surface: "discord" });
    const result = await handleCommands(params);
    expect(result.shouldContinue).toBe(false);
    expect(result.reply?.text).toContain("Models (ollama)");
  ...
  it("errors on out-of-range pages", async () => {
    // Use discord surface for text-based output tests
    const params = buildParams("/models ollama 4", cfg, { Surface: "discord" });
    const result = await handleCommands(params);
    expect(result.shouldContinue).toBe(false);
