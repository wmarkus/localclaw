...
describe("commands registry args", () => {
  ...
  it("serializes args via raw first, then values", () => {
    ...
    expect(serializeCommandArgs(command, { raw: "gpt-oss-120b-codex" })).toBe("gpt-oss-120b-codex");
    expect(serializeCommandArgs(command, { values: { model: "gpt-oss-120b-codex" } })).toBe(
      "gpt-oss-120b-codex",
    );
    expect(buildCommandTextFromArgs(command, { values: { model: "gpt-oss-120b-codex" } })).toBe(
      "/model gpt-oss-120b-codex",
    );
  });
