...
const usageMocks = vi.hoisted(() => ({
  ...
  formatUsageWindowSummary: vi.fn().mockReturnValue("Local 80% left"),
...
describe("trigger handling", () => {
  it("filters usage summary to the current model provider", async () => {
    await withTempHome(async (home) => {
      ...
      expect(normalizeTestText(text ?? "")).toContain("Usage: Local 80% left");
      expect(usageMocks.loadProviderUsageSummary).toHaveBeenCalledWith(
        expect.objectContaining({ providers: ["ollama"] }),
      );
    });
  });
