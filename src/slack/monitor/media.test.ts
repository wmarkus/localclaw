...
describe("resolveSlackMedia", () => {
  ...
  it("prefers url_private_download over url_private", async () => {
    ...
    expect(fetchRemoteMedia).toHaveBeenCalledWith(
      expect.objectContaining({ url: "https://files.slack.com/download.jpg" }),
    );
  });
  ...
  it("falls through to next file when first file returns error", async () => {
    ...
    expect(fetchRemoteMedia).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ url: "https://files.slack.com/first.jpg" }),
    );
    expect(fetchRemoteMedia).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ url: "https://files.slack.com/second.jpg" }),
    );
  });
});
