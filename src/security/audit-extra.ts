...
const WEAK_PARAM_THRESHOLD_B = 20;
...
export function collectModelHygieneFindings(cfg: OpenClawConfig): SecurityAuditFinding[] {
  ...
  for (const entry of models) {
    ...
    if (paramB !== null && paramB < WEAK_PARAM_THRESHOLD_B) {
      addWeakMatch(entry.id, entry.source, `Below ${WEAK_PARAM_THRESHOLD_B}B parameters`);
    }
  }
  ...
  if (weakMatches.size > 0) {
    ...
    findings.push({
      ...
      detail:
        ...
        lines +
        more,
      remediation: `Prefer models at or above ${WEAK_PARAM_THRESHOLD_B}B parameters for any bot with tools or untrusted inboxes.`,
    });
  }
...
function isWebSearchEnabled(_cfg: OpenClawConfig, _env: NodeJS.ProcessEnv): boolean {
  return false;
}
